...
function Crop(options) {
  ...
  function _upload_image(img_name, img_data) {
    ...
    fetch('/upload', options).then(function (response) {
      return response.text();
    }).then(function (uploaded_filename) {
  ...
  function _side_move(e) {
    if (cur_side.index == 1) {
      ...
      if (_top < preview_inner.clientHeight && _top > 0) {
        ...
        if (options.ratio) {
          var _old_width = self.crop_area.width;
      ...
      if (_left < preview_inner.clientWidth && _left > 0) {
        ...
        if (options.ratio) {
          var _old_height = self.crop_area.height;
  ...
  function _update_preview() {
    ...
    var scale = img_el ? img_el.naturalWidth / img_el.clientWidth : 1,
        img_options = {
      img: self.img_url
    };
  ...
  function generate_crop_template() {
    ...
    var _upload_img = document.createElement('img');
    ...
    var _remove_img = document.createElement('img');
